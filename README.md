...
QUIC (UDP 443) по умолчанию блокируется: через туннель он ведёт себя хуже TCP —
потери множатся, а браузер не понижает скорость. После блокировки браузер сам
откатывается на HTTP/2 поверх TCP. Так делают все клиенты прокси.
