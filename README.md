...
Три режима: выключено, «список мимо VPN» и «только список через VPN».

Первый нужен играм с античитом. Они сверяют, откуда вы заходите, и смена страны
посреди сессии выглядит для них как угон аккаунта — за это блокируют. Программа,
внесённая в список исключений, идёт напрямую и не попадает ни под правила обхода,
ни под фильтры: её адрес остаётся вашим настоящим. Второй режим — обратный: в
туннеле только перечисленное, всё прочее идёт мимо.

Программа опознаётся по имени исполняемого файла. Регистр значения не имеет, и это
не мелочь: поле `process_name` в ядре сравнивает имя посимвольно, поэтому запись
`ESCAPEFROMTARKOV.EXE` **не** совпадала с файлом `EscapeFromTarkov.exe` — правило
молча не срабатывало, а человек был уверен, что игра выведена из туннеля. Поэтому
здесь используется `process_path_regex` с флагом `(?i)`, привязанный к имени файла:

    (?i)[\/]escapefromtarkov.exe$

Проверено на живом ядре: соединение из программы с таким именем уходит в `direct`
в режиме исключения и в `proxy` в режиме «только список», а обычная программа — наоборот.
Полный путь указывать не нужно, правило переживает переустановку игры на другой диск.
Обратная сторона: две разные программы с одинаковым именем файла попадут под правило обе.

## Язык

Русский и английский, переключение в настройках; по умолчанию — как в системе.
Ключ словаря — сама русская строка, поэтому потерять текст невозможно: если перевода
нет, показывается оригинал. Словарь один на окно и на главный процесс
([src/renderer/i18n.js](src/renderer/i18n.js)), там же живут сообщения о состоянии
подключения, ошибки кабинета и названия модулей.

## Обновление

Проверка идёт через две с половиной секунды после запуска и дальше раз в шесть часов.
Пока идёт первая проверка, внизу окна видна полоса «проверяю обновления» — она же
потом скажет, что вышла новая версия. Если нового нет, полоса просто исчезает и
больше не появляется: мигать ею каждые шесть часов незачем.

**Обновление в одно нажатие.** Кнопка на полосе (или в панели «Обновление») делает
весь путь сама: скачивает, сверяет контрольную сумму, ставит и перезапускает
приложение. Второго подтверждения нет — человек уже сказал «обнови». Пока идёт
загрузка, полоса показывает «подождите чуть-чуть» и процент, а заливка идёт фоном
самой полосы. Рядом кнопка отмены: прерванная загрузка не ставится и файл удаляется.

Порядок обращений проверен на стенде с записью вызовов:

    обновление найдено   → скачать → поставить
    файл уже скачан      → поставить (заново не качает)
    ничего не известно   → проверить → скачать → поставить
    нового нет           → проверить и остановиться
    отмена или ошибка    → без установки

В меню при этом загорается точка на кнопке и появляется строка «Обновление».

Установка идёт поверх текущей версии. Ключи, вход в кабинет и настройки лежат в
`%APPDATA%EVA VPN` и переустановку переживают. Если туннель был поднят до
обновления, новая версия поднимет его сама — приложение оставляет себе метку и
читает её при первом запуске, там же берётся текст благодарности.

Манифеста `latest.yml` в релизе нет и не будет: electron-updater не используется,
всё нужное GitHub отдаёт в API. Оттуда же берётся **sha256 установщика** — файл
качается с одного хоста, контрольная сумма приходит с другого, и если они не сойдутся,
загруженное удаляется и ничего не ставится. Ссылка принимается только вида
`https://github.com/theEVAai/EVA-VPN/releases/download/...` или с хостов раздачи
GitHub — подсунуть чужой адрес ответом API нельзя.

Чего здесь нет: проверки подписи. Приложение не подписано (см. «Неизвестный
издатель» ниже), поэтому подлинность держится на TLS до GitHub и на контрольной
сумме. Подпись это заменит, когда появится сертификат.

## Расширение для браузера

Лежит в [extension/](extension/), собирается командой `node scripts/pack-extension.mjs`
в архив для Chrome Web Store и Firefox Add-ons. Manifest V3, один код на оба браузера.

**Чего оно не делает.** Расширение не поднимает VLESS/Reality и не может: браузер не
даёт расширениям работать с сокетами и шифрованием на таком уровне. Любое расширение,
обещающее «VPN внутри браузера» без своего клиента, на деле гонит трафик в чужой прокси.

**Что делает.** Направляет трафик вкладок в локальный прокси, который поднимает
настольное приложение (`127.0.0.1:2080`). Это защита только браузера: без прав
администратора, без виртуального адаптера, остальные программы идут напрямую. По сути
обратная сторона раздельного туннелирования — там вы выносите программы из туннеля,
здесь наоборот заводите в него одну.

Состояние приложения расширение узнаёт у того же Clash API (`127.0.0.1:19090`) и
показывает честно: если клиент не запущен, в окне так и написано, а не «подключено».

Два браузера — два разных API, обе ветки в [background.js](extension/background.js):
Chromium ставит одну настройку прокси на весь браузер, Firefox решает по каждому
запросу. Список исключений («мимо туннеля») работает одинаково в обоих, проверен
десятью случаями, включая `*.local` и попытку выдать `evil-example.com` за
`example.com`.

Архив собирается своим упаковщиком, а не средствами Windows: и `Compress-Archive`,
и `ZipFile.CreateFromDirectory` пишут пути с обратными слэшами, а формат ZIP требует
